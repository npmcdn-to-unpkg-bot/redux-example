...
describe('components', () => {
  describe('TodoItem', () => {
    ...
    it('edit state render', () => {
      ...
      expect(input.type).toBe(AddTodo)
      expect(input.props.text).toBe('Use Redux')
      expect(input.props.editing).toBe(true)
    })

    it('AddTodo onSave should call editTodo', () => {
      const { output, props } = setup(true)
      output.props.children.props.onSave('Use Redux')
      expect(props.editTodo).toHaveBeenCalledWith(0, 'Use Redux')
    })

    it('AddTodo onSave should call deleteTodo if text is empty', () => {
      const { output, props } = setup(true)
      output.props.children.props.onSave('')
      expect(props.deleteTodo).toHaveBeenCalledWith(0)
    })

    it('AddTodo onSave should exit component from edit state', () => {
